...
- **Binance Spot.** Choose **Binance** in Import market data to search public Spot pairs such as BTCUSDT, with no API key. Choose 1m, 5m, 15m, 30m, 1h, 4h, 1d or 1w candles, a preset or custom UTC date range, and a folder (default Binance). Max downloads all history available from the public API; only completed candles are saved, and a date-only end includes that entire UTC day. Requests are paginated and rate-limited, and verified pages are checkpointed locally so repeating the same request resumes after interruption. All pages become one library CSV named `BINANCE_SPOT_<PAIR>_<TIMEFRAME>`, separate from LSE datasets. Prices are in the quote asset and volume is in the base asset; for example BTCUSDT is priced in USDT with volume in BTC. This imports Spot OHLCV only; it does not simulate perpetual funding or derivatives contract economics. Binance network/location restrictions are reported in the dialog.
